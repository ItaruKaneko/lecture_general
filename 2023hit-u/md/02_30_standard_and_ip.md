...
Multics は一部PL/Iなどのコンパイラで書かれていた。

PL/Iなどのコンパイラは当時手作業で作るのが普通だった。
(コンパイラ自体はコンパイラで書かれていない)

他機種に移植するのはきわめて困難。

UNIXはC言語で書かれ、Cコンパイラ自体もC言語で書かれていた。

オブジェクト生成部分を他コンピュータ用に書き換えれば、コンパイラの委嘱が完了する。

コンパイラが移植できればUNIXも移植できる。

つまり別のコンピュータに、C言語とUNIXをわずかな作業で移植できる。

このような性質をポータビリティと呼び、UNIXがポータブルになったということはUNIXがPDP-11以外に容易に移植できるようになったということ。
...
1976～1983 に Version 6, version 7が公開されると爆発的に多くのコンピュータにUNIXが移植された。

その後System V, BSD, POSIX などの標準やリソースが整備された。
